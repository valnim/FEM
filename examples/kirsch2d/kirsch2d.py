...
q0 = 100
lam = 1.5
top_surface_force1 = lambda x, y: [0, q0]
top_surface_force2 = lambda x, y: [-q0*lam, 0]
...
class MyBCHandler(BCHandler):
    def integrateBC(self):
        ...
        for face in self._model.getBoundary(top_boundary1).component_list:  # Neumann Top
            for int_point in face.int_points:
                int_point.setSurfaceLoad(top_surface_force1)

        for face in self._model.getBoundary(top_boundary2).component_list:  # Neumann Top
            for int_point in face.int_points:
                int_point.setSurfaceLoad(top_surface_force2)
