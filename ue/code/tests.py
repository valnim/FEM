...
def test_numint_2d():
    ...
    for ip in integration_points:
        print(ip.undeformed_coordinates)
        print(ip.weight)
        print()
...
def test_new_implementation():
    ...
    E_mod = 210000
    nu = 0.3
    material = LinearStVenantKirchhoffMaterial(number, E_mod, nu, 'plane_stress')
    element.material = material
...
def test_hex_shape_interpolation():
    import numpy as np
    import soofea.model.shape

    shape = soofea.model.shape.HexShape(1)
    everything_correct = True

    f = lambda node_id: 2 * node_id - 1
    for idx in range(2):  # Node index
        for jdx in range(2):  # Node index
            for kdx in range(2):  # Node index
                for ldx in range(-1, 1, 2):  # Natural Coordinates
                    for mdx in range(-1, 1, 2):  # Natural Coordinates
                        for ndx in range(-1, 1, 2):  # Natural Coordinates
                            if f(idx) == ldx and f(jdx) == mdx and f(kdx) == ndx:
                                if shape.get([ldx, mdx, ndx], [idx, jdx, kdx]) != 1:
                                    everything_correct = False
                            else:
                                if shape.get([ldx, mdx, ndx], [idx, jdx, kdx]) != 0:
                                    everything_correct = False

    print(f"test_hex_shape_interpolation tests ok: {everything_correct}")
